...
def get_input():                              
    ...
    shipment_quote["international"] = input("International (y/n): ")  # I am not assuming that ocean is equivalent to international
...
def ocean(shipment_quote):                                                                        # ocean
...
def air(shipment_quote):                                                            # air
    #print("Can this be shipped by air?")
    if ((shipment_quote["weight_kg"] >= 10) or (shipment_quote["cubic_meters"] >= 125)):          # too heavy, too voluminous
        shipment_quote["can_ship"] = "n"                                                          # I wrote it the way I wrote it bc it didn't repeat code
    else:                                                                                         # now all air together, all ocean together but code repeats
        shipment_quote["can_ship"] = "y"                    
    if shipment_quote["weight_kg"] >= 7:                                            
        shipment_quote["heavy"] = "y" 
    else:
...
def ground(shipment_quote):                                                          # ground
...
def main():                                      
    while True:
        ...
        if action == "1":
            ...
            shipment_quote = ocean(shipment_quote)
            #print("after ocean: ", shipment_quote)
            shipment_quote = air(shipment_quote)
            #print("after air: ", shipment_quote)
            shipment_quote = ground(shipment_quote)
...
# I don't like repeating the same code in ocean, air, ground.  I had it separate before.  Does that part get separated out again?
